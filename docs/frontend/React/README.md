...
- `shouldComponentUpdate`  这个方法有两个参数(nextProps, nextState)，表示新的属性和变化之后的 state，返回一个布尔值，默认返回 true 表示会触发重新渲染，false 则不会触发。我们通常利用此生命周期来优化 React 程序性能。
...
## 请求应该放在哪个生命周期中？
异步请求到底应该放在 React 的哪个生命周期里？有人认为在 componentWillMount 中可以提前进行异步请求数据，达到避免页面白屏的效果，其实这个观点是有问题的。

因为 JavaScript 中异步事件的性质，当进行 API 调用时，浏览器会在此期间返回去执行其他工作。所以当 React 渲染一个组件时，它不会等待 componentWillMount 完成任何事情，而是继续进行 render，即不会“暂停”渲染以等待数据到达。

而且在 componentWillMount 请求会有一系列潜在的问题：

- 在 SSR 服务器渲染时，如果在 componentWillMount 里获取数据，fetch data 会执行两次，一次在服务端一次在客户端，这造成了多余的请求。
...
在 `React Diff` 算法中`React` 会借助元素的 `Key` 值来判断该元素是新近创建的还是被移动而来的元素，从而减少不必要的元素重渲染，提升性能。所以在开发过程中，我们需要保证某个元素的 `key` 在其同级元素中具有**唯一性**。
