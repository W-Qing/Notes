...
众所周知，操作 DOM 是很耗费性能的一件事情。因而减少 DOM 操作也是前端性能优化的一个关键点。其实不仅仅是操作 DOM 相对较慢，更因为频繁变动 DOM 会造成浏览器的回流或者重绘，这些都会特别耗费性能。因此我们通过抽象出虚拟 DOM，即利用 JS 对象来模拟 DOM 对象，在 patch 过程中尽可能地将差异一次性更新到 DOM 中，这样保证了 DOM 不会出现性能很差的情况。
